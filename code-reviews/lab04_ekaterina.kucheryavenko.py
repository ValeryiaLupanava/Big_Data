...
# Привет! 
# Необходимо добавить комментарии, что происходит на каждом шаге и почему выполняется то или иное действие
# работа с train

train = pd.read_csv(r'/data/share/lab04data/lab04_train.csv')  
train = train[train['TARGET'].notnull()]
X = train
# Почему решила удалить все категориальные столбцы? Не приводила их к числовому представлению с помощью OnehotEncoding или др.
...
'PACK']
# Хорошо было бы проанализировать другие столбцы
# К примеру, удалить id, так как он не влияет на качество модели
...
# сразу обучение
# Как были подобраны параметры? Можно было использовать GridSearch
